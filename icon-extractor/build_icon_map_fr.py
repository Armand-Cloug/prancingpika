...
START = f"{BASE}/fr/abilities.jspa"

ABILITY_ICONS_TS = Path("build-ability-icons.ts")  # adapte si besoin
OUT_JSON = Path("icon_to_fr.json")
OUT_MISSING = Path("missing_icon_keys_fr.txt")

# /fr/abilities/2/Saigneur-des-Failles
SOUL_LINK_RE = re.compile(r"^/fr/abilities/\d+/.+")
# /fr/ability/1692245236/Aiguille-d-ombre
ABILITY_HREF_RE = re.compile(r"^/fr/ability/\d+/")
...
def extract_pairs_from_rendered_soul(html: str) -> list[tuple[str, str]]:
    ...
    <tr>
      ...
      <td><a href="/fr/ability/...">Nom FR</a></td>
      ...
    </tr>
    """
    soup = BeautifulSoup(html, "html.parser")
    ...
    for tr in soup.select("table.related tbody tr"):
        a = tr.select_one('td:nth-of-type(2) a[href^="/fr/ability/"]')
        if not a:
            # fallback si l'ordre change
            a = tr.select_one('a[href^="/fr/ability/"]')
        if not a:
            continue

        name_fr = a.get_text(" ", strip=True)
        if not name_fr:
            continue
...
async def main():
        ...
        async def process_soul(url: str):
            async with sem:
                ...
                try:
                    ...
                    for icon_key, name_fr in pairs:
                        if icon_key not in keys:
                            continue
                        lst = icon_to_fr.setdefault(icon_key, [])
                        if name_fr not in lst:
                            lst.append(name_fr)
                finally:
                    await p2.close()
    ...
    OUT_JSON.write_text(json.dumps(icon_to_fr, ensure_ascii=False, indent=2), encoding="utf-8")
    missing = sorted(k for k in keys if k not in icon_to_fr)
    OUT_MISSING.write_text("\n".join(missing), encoding="utf-8")

    print("\n--- Résumé ---")
    print(f"icon_keys matchées: {len(icon_to_fr)}")
